...
# 캡처 클리핑 적응 재캡처: peak가 CLIP_PEAK 이상이면 play-gain을 낮춰 CLIP_TARGET
# 근처로 재캡처(loss가 rms정규화라 톤 동일·클리핑만 제거). 클린 프리셋은 출력이
# 입력에 비례해 한 번에 풀리고, 포화(디스토션) 프리셋은 안 풀릴 수 있어 횟수 제한.
CLIP_PEAK = 0.98
CLIP_TARGET = 0.8
MAX_CLIP_RECAPTURE = 2
...
class ReampEvaluator:
    ...
    def _recapture_unclipped(self, rec, sr, peak):
        """클리핑 시 play-gain을 임시로 낮춰 재캡처(HID 재적용 없이 — 후보는 장치에
        이미 적용됨). 풀리면 비클리핑 캡처를 반환. Stage1·2 공통(__call__ 경유)."""
        saved = self.play_gain
        try:
            for _ in range(MAX_CLIP_RECAPTURE):
                self.play_gain *= CLIP_TARGET / peak    # peak→CLIP_TARGET 목표로 비례 축소
                r, s = self.reamp()
                m = r.mean(axis=1) if r.ndim > 1 else r
                p = float(np.max(np.abs(m)))
                rec, sr, peak = r, s, p
                if peak < CLIP_PEAK:
                    break
        finally:
            self.play_gain = saved                       # 다음 trial은 calibrate된 값 유지
        return rec, sr, peak
    ...
    def __call__(self, candidate):
        ...
        # 클리핑 적응 처리: 클리핑은 캡처 파형을 깎아 loss를 오염시킴 → play-gain을
        # 임시로 낮춰 재캡처(loss는 rms정규화라 톤 동일). Stage1·2 모두 여기서 처리됨.
        if peak >= CLIP_PEAK:
            pre = peak
            rec, sr, peak = self._recapture_unclipped(rec, sr, peak)
            if peak >= CLIP_PEAK:                 # 포화 프리셋 → 입력 낮춰도 안 풀림
                print(f"  [WARN] capture still clipping (peak={peak:.3f}, was {pre:.3f}) "
                      "after gain reduction — 포화 프리셋(메이크업/타겟 더 낮춰야)", flush=True)
            else:
                print(f"  (clipping {pre:.3f}→{peak:.3f} resolved by gain reduction)",
                      flush=True)
