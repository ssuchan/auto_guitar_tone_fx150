...
USB 재생(스피커 FX150) → 캡처(마이크 FX150) 신호 RMS=0 → **USB 재생은 이펙트 우회.**
...
- [x] Phase 3: 유튜브 다운로드 + Demucs 분리 (fetch_separate.py) — 코드 완성, 실URL 미실행
...
- 단계형 최적화(거친 모델탐색→미세 노브) 도입해 AMP 58종 탐색 효율화
