...
5. **게인 레벨**(곡 성격: clean/crunch/overdrive/distortion/metal)을 고른다 — 탐색 효율↑.
   잘 모르면 **auto(타겟분석)** 체크.
6. **[학습하기]** → 다운로드는 건너뛰고 바로 학습. 진행 로그가 실시간 표시(기본값 기준 약 10~20분, MOD 탐색 포함 시 더).
7. 끝나면 찾은 설정이 FX150에 적용되고, **저장 이름/슬롯**을 채웠으면 프리셋으로 저장된다.

> 결과가 마음에 안 들면 **"이전 best 이어 개선"** 을 켜고 다시 학습하면, 이전 결과를
> 출발점으로 더 좋은 톤을 누적 탐색합니다. 코어(앰프/EQ)는 그대로 두고 **모듈레이션만**
> 빠르게 얹고 싶으면 **"MOD만 (이전 best 위에)"** 를 켜세요(Stage 1/2 생략).
...
| 추가 탐색 — FX 컴프 | 컴프레서를 더하는 Stage 2(~4분). 기본 **꺼짐**(노이즈↑). 필요한 곡만 체크. |
| 추가 탐색 — 모드 MOD | 모듈레이션(코러스/트레몰로/피치 등)을 더하는 Stage 3(~7분). 기본 **켜짐**. |
| MOD 종류 | `자동(전수)`=20개 MOD 모델 전수 탐색. 특정 모델을 고르면 **그 모델 파라미터만** 학습(귀로 종류를 알 때, 빠름). |
| 횟수 | MOD 탐색 trial 수(기본 70). 모델을 직접 고르면 30 정도로 낮춰도 충분. |
| MOD만 (이전 best 위에) | 켜면 이 곡의 이전 best를 코어로 고정하고 **MOD만** 빠르게 탐색(Stage 1/2 생략). 이전 학습 결과 필요. |
...
3. **비교** — 처리된 소리와 목표 톤의 음색 차이를 perceptual loss(MFCC·스펙트럼 대비·
   대역별 밝기/밸런스 등)로 수치화. 밝기는 단일 비율이 아니라 **여러 주파수 대역으로 나눠**
   비교해, 옵티마이저가 한 대역을 부풀려 다른 대역을 죽이는(어둡거나 답답한 톤) 꼼수를 막는다.
4. **최적화** — Optuna(베이지안 최적화, TPE)가 loss가 줄어드는 방향으로 다음 설정을 제안.
   2~3을 수백 번 반복해 가장 비슷한 설정을 찾는다.
- **Stage 1**: 앰프/드라이브/캐비넷/EQ의 큰 틀. **Stage 2(선택)**: FX 컴프레서.
  **Stage 3(선택)**: 모듈레이션(MOD) — 20개 모델 전수 탐색, 또는 종류를 직접 골라 그 파라미터만.
- 딜레이/리버브는 시변 효과라 loss로 판단이 어려워 자동 탐색에서 제외하고, 별도 A/B로 고른다.
...
- 학습 중에는 그 컴퓨터로 **소리 나는 작업을 하지 마세요.** 컴퓨터→FX150로 보내는 소리에
  잡음이 섞이면 학습이 망가집니다.
- **Tap Tempo(딜레이 시간)와 딜레이/리버브는 자동 학습하지 않습니다** — 별도 A/B로 고르거나 직접 손보세요.
  (모듈레이션(MOD)은 Stage 3에서 자동 탐색하거나 종류를 직접 지정할 수 있습니다.)
...
# 학습 (Stage1 + MOD 전수 / FX 컴프는 끔)
python src/main.py --song <곡> --trials 100 --stage2-trials 0 --stage3-trials 70 --play-gain 0.4
# 이전 best 위에 MOD만 빠르게 (종류 직접 지정: 예 20=PITCH POLY)
python src/main.py --song <곡> --mod-only --mod-model 20 --stage3-trials 30
```

- `--mod-model N` — MOD를 N번 모델로 고정하고 파라미터만 세밀화(0=전수 탐색).
- `--mod-only` — 이전 best를 코어로 고정, Stage 1/2 생략하고 MOD만 탐색.
